...
public:
    int superPow(int a, std::vector<int>& b) {
        ...
        int result = 1;
        // For each digit i in b, we calculate a^b[i], multiply this with
        // the result, and exponentiate the result to the power of 10.
        // This means we split the computation as follows:
        // (((a^b[0])^10 * a^b[1])^10 * a^b[2])^10 ...
        // This way, each digit b[i] gets multiplied by its respective power of 10.
        for (int i = 0; i < b.size(); i++) {
            // Take everything to the power of 10, which prepares the result
            // for the next digit. This does nothing on the first iteration.
            int tmp = 1;
            for (int j = 0; j < 10; j++) {
                tmp = (tmp * result) % BASE;
            }
